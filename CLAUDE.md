...
**Not yet done:** wiring the turn into rotation *event* detection (so the entry screen verdicts on turn-started events). That would move the start bars the ENTRY_SCREEN thresholds were fitted on, and needs a stage-5 stock-level study first — enter member names at the turn date vs the quadrant date, 20d hold, breadth veto applied, over the 78 cached rotations.
