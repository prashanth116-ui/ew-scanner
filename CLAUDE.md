...
**Stale expiry** (`BAR_BUDGETS[tf].staleExpiry`) is separate from invalidation and load-bearing. FVG_CONFIRMED, BSL_BUILT and IGNITION are **absorbing**: each waits on something that may never come, and none invalidates when price walks *away*, because the protected low sits far below. A setup that has not progressed within the budget is abandoned so the engine hunts a fresh raid. Without it the first production run reported median ages of 297 bars at BSL and 577 at Ignition — names carrying a BSL target 85% below spot from 2000+ bars back — and those distances overflowed `distance_to_bsl_pct`, taking whole upsert batches with them (422 qualifying, 369 persisted). An expiry is **not** recorded as a `prior_invalidation_*`: nothing broke, so flagging it as a break would misreport the risk.
...
**Entry quality reads `entryRetracement`, not current position.** ICT has two entry models — the retracement into a PD array, and the expansion through liquidity — and the ladder covers both. ARMED *requires* price within 3% of the draw, so it is in premium **by construction**; grading it on where price sits now scored every state the page exists to surface at close to zero, and flagged all of them `is_late_entry`, leaving Top Picks permanently empty. `entryRetracement` is the deepest discount reached since the MSS — "did this setup ever offer an entry" — which measures the same idea without contradicting the ladder. For the same reason `detectLateEntry` applies the FVG-distance test only below BSL_BUILT, and premium is surfaced as its own badge rather than a disqualification.

**Transition lookups must be scoped to the current setup.** `transitions` is an audit trail and deliberately survives a reset, so a plain `.find()` returns the first match ever recorded — possibly from a setup that died hundreds of bars ago. `currentTransition()` searches backwards and requires `barIndex >= sslBarIndex`.
...
**Thin baskets:** UFO carries 3 members after the 2026-08-08 micro-cap trim and the 2026-09-22 delisted-ticker trim, so it reports null breadth by design; reaching the minimum would mean re-adding the trimmed names. ARKX (7), KRE (6) clear it. Sub-sector baskets mostly do not own their members canonically (ARKX owns 0, AIQ owns 6 of 56) — that is fine for Tier 1, which counts listed members regardless of ownership, but it empties their Tier-2 pool.
...
- **Delisted-ticker trim 2026-09-22:** 27 symbols removed (30 listings, plus 3 now-orphaned `PRIMARY_SECTOR` pins) after `scripts/audit-sector-assignments.mjs` found they return nothing from either Yahoo endpoint the app uses. Breadth-neutral **by construction** rather than by careful selection — a symbol with no quote already drops out of `quotesInSector`, so it was contributing to neither numerator nor denominator. Verified anyway: `resolvable` counts and breadth percentages identical across all 22 baskets, no Tier-1 eligibility changes. The ratio-matched procedure above is for trimming **live** names, where composition genuinely does move breadth; it does not apply here.
- **Sector membership drifts as names delist.** 26 of those 27 were basket-only — `index-tiers.ts` gets maintained when the index changes and `sector-universe.ts` does not, so dead names accumulate here and nowhere else. `node scripts/audit-sector-assignments.mjs` re-checks every symbol against Yahoo's reported sector and flags the unresolvable ones; `--parse-only` runs its self-checks offline in a second.
...
**Rotation Turn** (`rotation-turn.ts`, `ROTATION_TURN` in config): a **dated** RS inflection reported beside `quadrant`, never folded into it. `calcRRG` stacks a 10-period EMA → 200-bar z-score → 10-period ROC → second z-score, so RS-Momentum cannot cross 100 until a turn is weeks old. SMH 2026-09: RS line bottomed 09-14, reclaimed its 20d SMA 09-17, its 50d 09-18 — the quadrant printed LEADING on 09-21, after most member names had moved.

Stages `NONE → TURN_FORMING → TURN_DETECTED → TURN_CONFIRMED → QUADRANT_CONFIRMED` (`TURN_STAGE_ORDER` for comparisons), plus `rsLowDate` / `formingDate` / `turnDate` / `confirmedDate` / `quadrantDate` and `quadrantLagBars`.

**`TURN_FORMING` is the watchlist stage and the only one that fires before a reclaim** — RS line still *below* its fast SMA, but risen two sessions with the gap narrowing each time. SMH: forming 09-16, reclaim 09-17, slow-SMA confirm 09-18, quadrant 09-21. Entry-to-09-21 relative returns were +6.67% / +4.89% / +2.47% / 0.00% respectively, so the session matters. **It is not a position signal and the copy must never imply one:** 1,895 fires over 37 ETFs × 3y (12.7/week board-wide), under half followed by a reclaim within 5 sessions, forward returns +0.02%/3d, −0.04%/5d, −0.14%/10d, −0.04%/20d against a control of +0.03/+0.04/+0.08/+0.19. `turnDate`, `confirmedDate` and `quadrantLagBars` are all null at this stage by design — a forming setup must not be readable as a completed one. `FORMING_RISING_SESSIONS: 2`; raising it to 3 cuts volume to 4.0/week but stops firing on 09-16 at all. Only the UP direction forms — the mirror would fire on any two-day dip inside an uptrend and would clobber the turn dates of every sector in one.

**REJECTED: breadth velocity as a filter on `TURN_FORMING`** — do not re-propose without new evidence. Measured on 446 forming events across the 21 baskets with >= 5 members (2024-11-19 → 2026-07-21, breadth computed as ENTRY_SCREEN computes it): correlation between 3-session breadth velocity and forward 10d relative return is **0.016**. Top velocity quartile lifts reclaim-within-5 from 43% to 54% but its forward returns are indistinguishable from control; `vel3 >= +15pp` is *worse* than `>= +10pp` on every horizon (a threshold reversing as it tightens is noise); and the exact SMH-09-16 shape — velocity >= +10pp with level < 45% — is historically a **loser** (n=21, −0.99%/20d, 33% positive). Unexpected same-run result worth a real study but **not** a change: breadth *level* at the forming date looks **inverted** — bottom quartile +0.65%/20d at 58% positive vs top quartile −0.60% at 37%, and events clearing the 60% gate ran −0.61% at 37%. Measured at a different bar than ENTRY_SCREEN's rotation start, so not a contradiction of that study; one slice among ~20 examined, so a hypothesis. Carried on `SectorRotationScore.rotationTurn` and `RotationTrackerResult.rotationTurns` (keyed by sector id — an event is a closed period, the turn is a live read). Surfaced on `/sectors` cards, the expanded-card timeline, `/rotation` cards and the `/sectors/picks` entry-signals panel, and **alerted** as the RS Turns block heading the 03:02 UTC rotation × scanner confluence message — see `src/app/api/CLAUDE.md`.

⚠️ **A faster quadrant was measured and rejected — do not re-propose without new evidence.** 37 ETFs, 3y of point-in-time quadrants (~27k sessions), eleven candidate triggers: median lead over the shipped quadrant was **0-1 sessions**, signals went 751 → 1,008-1,823, 5-session reverts 17% → 34-61%, median run 14d → 4d. Forward 20d return vs SPY after promotion: control 47.7% positive / +0.19%, shipped quadrant 45.6% / −0.05%, every fast rule 45-47% / ~0% — **promotion carries no standalone edge at either speed**, matching `scripts/rotation-backtest/README.md` ("the gate alone is worthless… the veto does most of the work"). So the turn is a dated read, not a score or a gate.

Two things the module is deliberately honest about: `rsLowDate` is **retrospective** (on 09-14 SMH underperformed SPY by 4.31% with its RS line 2.83% under the 20d — no live rule fired there), and `priorFailedAttempts` reports the cost of the speed (SMH reclaimed its 20d five times in seven weeks; 08-07, 08-12, 08-27 and 09-04 all failed). `rsLowDate` is the **excursion** low the reclaim came off; `legLowDate` is the wider lookback low — SMH prints 09-14 and 07-29. `quadrantLagBars` is never negative: a quadrant already in the bucket sets `quadrantAlreadyAligned` instead.

**`ROTATION.QUADRANT_GUARD_RESPECTS_TURN`** (true) lifts the 5-day RS-golden-cross suppression when the turn is UP and confirmed. The guard's premise — RS strength contradicting the quadrant is noise — inverts during a rotation start, and on the 2026-09-21 board it was suppressing XBI, XLV and XLE. Safe because the guard only touches the last `QUADRANT_GUARD_DAYS` bars: a suppressed bar is unsuppressed once it ages out and `detectRotationEvents` dates the event from that same bar, so **start dates and the bar ENTRY_SCREEN measures on are unchanged** — only live recognition moves, by up to 5 sessions.

**Measured and rejected: moving `startDate` onto the turn, or letting the turn drive the timing tiers.** `scripts/rotation-backtest/5-turn-vs-start.mjs`, 64 paired rotations, median lead 5 sessions. The ETF captures more from the turn bar (+6.59% vs +4.71% forward 20d) and the member stocks capture **less** (68% win / +12.04% vs 77% / +13.39%), because at the turn bar the members have not confirmed: gate passes 7/64 vs 22/64, mean breadth 49.7% against a 60% gate, 1.5 qualifying names vs 2.2. The turn is early for the sector and too early for its members — a notification device, not an entry device. Showing a young rotation earlier is supported; entering earlier is not, which is exactly the split the picks panel implements.

**Centralized Config:** All thresholds in `src/lib/sector-rotation/config.ts`. Sections: REGIME, COMPOSITE, ROTATION_TURN, ROTATION, QUALITY_GATES, CONVICTION, LEADERSHIP, RISK_FLAGS, POSTURE, SMART_MONEY, TOP_STOCK_WEIGHTS, CLASSIFICATION, SCORING_SIGNALS, ROTATION_LIFECYCLE, ROTATION_CONVICTION, SUB_SECTOR, CRYPTO_QUALITY_GATES, EXTENSION_TIERS, PRERUNNER, CRYPTO_WEIGHTS, PREMARKET_SCORING. **Never hardcode thresholds** — always add to config.ts and import.
...
- **`GET /api/cron/hit-rates`** (03:20 UTC Tue-Sat) — persists forward-return hit rates for **Inflection and Transition** into `scanner_hit_rates`. `?engine=`, `?lookback=`, `?dryRun=true`. Logic in `src/lib/backtest/daily-hit-rates.ts`.
- **Read `avg_excess_return_pct`, not `avg_return_pct`.** An absolute average return is uninterpretable without the tape: Transition's `is_stronger` signals averaged −4.5% over the three weeks to 2026-09-22 while QQQ returned +5.1%. Migration 034 adds the benchmark (SPY over the identical holding window), median, win-rate-vs-benchmark and the sample date span.
- **Entry is the close ON `scan_date`, not the row's `price` column.** That column holds the *prior* session's close — the 01:45/01:55 UTC crons read yesterday's bar — so pricing entries off it books a fill nobody could have taken. Exit prices come from Yahoo rather than the table, because a ticker only appears on days it qualified and reading exits from the table would restrict the sample to names that kept qualifying.
- **Two tables, two definitions of "hit".** `source = 'signal_outcomes'` rows (ew/squeeze/confluence/prerun, written by `/api/cron/outcomes`) score target hits; `source = 'daily_table'` rows score forward returns. Do not average them together.
- **`total_signals` on a `daily_table` row counts EPISODES, not ticker-days** (migration 035). A name sitting in one state for 15 sessions used to contribute 15 overlapping windows on the same move — which did not merely inflate `n`, it weighted the mean toward names that persist in the scan, i.e. the ones already working. Collapsing to state entries moved Transition's 14d all-signal excess from −0.78% to −1.61% and turned `HIGHER_LOW_FORMATION` from the board's only positive bucket (+0.88%) into a losing one (−1.63%). Compare `distinct_tickers` against `total_signals`: close together means independent names, far apart means repetition.
- **Never blend `scanner_version`.** `is_stronger` requires `runner_score >= 50` and V2 rows carry `runner_score: 0`, so the flag is not the same predicate across the Aug 19 2026 boundary. `computeDailyHitRates()` scopes to the newest version present and records it on every row.
