...
**`isSignalSustained()` returns `boolean | null`, and null is not false.** Null means too little history to judge (under 3 bars); false means measured and failed. Collapsing them rejected a rotation the tracker had only just noticed — SMH on 2026-09-22 carried one bar of history and a signalCount of 3, so the average passed easily and the length guard vetoed it anyway. A confirmed RS turn waives the null case, exactly as it waives the age filter; a measured `false` is never waived. This is the same null-vs-false rule the root CLAUDE.md sets for shared features, and fixing the age filter alone was not enough — SMH stayed hidden until both filters were made turn-aware.
...
**RS Turns block** (`computeBriefRsTurns()` in `brief.ts`, rendered by `RsTurnsBlock` above Quadrant Transitions): sectors whose RS line turned within `RS_TURN_WINDOW_SESSIONS` (3), ETF-level, capped at 5. It leads the What Changed section because the quadrant trails the RS line by 3-5 sessions, so on the mornings that matter it is the only entry with anything to say — SMH carried `FORMING 2026-09-16` on the 09-17 brief and `TURNED 09-17` on the 09-18 brief while Quadrant Transitions stayed empty until 09-21.

⚠️ **Computed OUTSIDE `computeWhatChanged()`, deliberately.** That function diffs against the previous localStorage snapshot and early-returns `noHistory: true` when there isn't one — first visit, cleared storage, or a snapshot past the 3-day staleness guard. A turn needs no diff: `turnDate` / `formingDate` come from the price series. Folding it in would blank the block exactly when a reader has been away and the catch-up value is highest, so `RsTurnsBlock` renders in the `noHistory` and zero-changes branches too.

**Windowed, not same-session** — unlike the Telegram block, which fires once so it cannot repeat nightly. The brief is a daily read, so a Monday turn is still news on Wednesday if that is the first morning you opened it. Sorted lead-bearing first, then freshness: an `alreadyAligned` re-entry ranks last however recent, because the 6 PM alert already covered it. **ETFs listed here are removed from Quadrant Transitions** via `rsTurnEtfs()` — one event seen by two constructs at different speeds, and printing both reads as two separate things happening.
