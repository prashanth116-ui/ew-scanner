...
export function SummaryStrip({ data, sectors }: { data: SectorRotationResult; sectors: SectorRotationScore[] }) {
  ...
  const dispLabel = data.dispersionIndex > 4 ? "High" : data.dispersionIndex > 2 ? "Moderate" : "Low";
  const spreadLabel = (data.sectorSpread ?? 0) > 8 ? "Wide" : (data.sectorSpread ?? 0) > 4 ? "Moderate" : "Narrow";
  ...
  return (
    <div className="flex items-center gap-4 rounded-lg border border-[#2a2a2a] bg-[#141414] px-4 py-2 overflow-x-auto">
      ...
      {/* Rotation */}
      <div className="flex items-center gap-2 shrink-0" title={data.rotationSummary ?? undefined}>
      ...
      <div className="flex items-center gap-1.5 shrink-0">
        <span className="flex items-center gap-0.5 text-[10px] text-[#555]">Disp <InfoTip text={`How spread out sector scores are \u2014 higher means more rotation opportunity. Current: ${dispLabel}`} /></span>
        ...
        <span className={`text-[9px] ${
          data.dispersionIndex > 4 ? "text-green-400/60" : data.dispersionIndex > 2 ? "text-amber-400/60" : "text-[#555]"
        }`}>{dispLabel}</span>
      </div>
      <div className="h-4 w-px bg-[#2a2a2a] shrink-0" />
      {/* Spread */}
      <div className="flex items-center gap-1.5 shrink-0">
        <span className="flex items-center gap-0.5 text-[10px] text-[#555]">Spread <InfoTip text={`Gap between best and worst sector scores \u2014 wider spread means clearer winners and losers. Current: ${spreadLabel}`} /></span>
        <span className={`text-sm font-bold font-mono ${
          (data.sectorSpread ?? 0) > 8 ? "text-green-400" : (data.sectorSpread ?? 0) > 4 ? "text-amber-400" : "text-[#888]"
        }`}>{(data.sectorSpread ?? 0).toFixed(1)}%</span>
        <span className={`text-[9px] ${
          (data.sectorSpread ?? 0) > 8 ? "text-green-400/60" : (data.sectorSpread ?? 0) > 4 ? "text-amber-400/60" : "text-[#555]"
        }`}>{spreadLabel}</span>
      </div>
      <div className="h-4 w-px bg-[#2a2a2a] shrink-0" />
      {/* Improving / declining */}
