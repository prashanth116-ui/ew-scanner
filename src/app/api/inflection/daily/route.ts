...
export async function GET(request: NextRequest) {
  ...
  const allDates = await loadInflectionDailyDates(14);
  ...
  // Load multi-day data for streak computation (all dates up to and including selected)
  const datesForStreaks = allDates.slice(currentIdx);
  const multiDayRows = await loadInflectionDailyMulti(datesForStreaks);
