...
**Detail budget scales with lead, but the tickers are never cut.** A lead-bearing turn gets the RS distance, low date, failure count and the named members. A re-entry gets two lines: header plus the ticker row. What was removed from re-entries is the prose — quadrant history, RS distance, failure count — all of which restated the section heading for a setup that by definition has no lead. The ticker row stays everywhere: "IGV reclaimed its 20d" is not a decision, "TEAM +33.6%, ZS below its 50d" is. A first trim cut the tickers too and had to be reverted.

**Scanner-flagged names are promoted and marked ⚡ in the turn block.** The turn block and the confluence body list the same basket through two different lenses — here every focus member ranked by distance above its own 50d, there only the names a scanner flagged tonight ranked by scanner agreement. Both are right, and they routinely share no tickers: IGV showed TEAM/OKTA/ZS/TWLO/NET/CRWD up top and SHOP/PLTR/SNOW/WDAY/DDOG below, all ten focus names. Two unlabelled lists for one basket reads as the message contradicting itself. `formatRotationTurns` takes the scanner-hit ticker set, sorts flagged names first, marks them ⚡ and prints the legend once — so one row carries both facts. A name can be below its 50d and still lead the row if a scanner flagged it (`·RDDT -2.7%⚡` above `✓GOOGL +2.7%`); the ✓/· still reports trend state, so nothing is hidden.

**The rotation lifecycle belongs on the turn line.** `Day 39 LATE` renders beside the reclaim date, sourced from `currentRotations`. Without it the message showed IGV as a fresh reclaim at the top and `Day 39 | LATE` forty lines below in the Monitor section, reading as two opposing claims about one basket. They are not contradictory — an old rotation dipped and bounced — but only if stated together.
...
**`priorFailedAttempts` is not in the alert.** Stage 8 measured it at −2pp on whether a turn matures, with 3+ failures scoring mildly *better* than 1-2 — the opposite of the intuition it shipped on. It led an alert detail line for one evening on no evidence. It survives as UI tooltip texture only; never restore it to a line, a badge colour or a gate.
...
⚠️ **Scope is TWO conditions, not one: a basket you trade AND one where a turn has been measured to lead somewhere.** `TURN_WEAK_BASKETS` in `config.ts` suppresses the second. Stage 11 (`11-per-sector.mjs`) found maturation ranges 39% to 0% across baskets and the pooled results of stages 5-10 hid it completely — XLP, XLRE, XLE, XLU and ITA produced **zero** turns with 3+ qualifying names in two years across 17-24 turn events each. Focus scoping alone kept XLC, ITA, XBI and XLV, which produced 7 such turns between them. It suppresses only baskets MEASURED to fail, never merely unmeasured ones — a thin sub-sector basket absent from the study keeps its alert, because "not measured" is not evidence of failure.
