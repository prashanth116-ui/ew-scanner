...
export async function GET(request: NextRequest) {
  ...
  if (searchParams.get("dates") === "true") {
    const dates = await loadTransitionDailyDates(14);
  ...
  const results = await loadTransitionDaily(date);

  // Load all available dates to compute streaks + deltas
  const allDates = await loadTransitionDailyDates(14);
  ...
  const multiDayRows = await loadTransitionDailyMulti(datesForStreaks);
  ...
  if (prevDate) {
    for (const r of multiDayRows) {
      if (r.scan_date === prevDate && !currentTickers.has(r.ticker)) {
        dropped.push({ ticker: r.ticker, prev_score: r.overall_score });
      }
    }
