...
export async function GET(request: NextRequest) {
  ...
  if (searchParams.get("dates") === "true") {
    const dates = await loadICTDailyDates(14);
  ...
  // Load all dates for streak/delta computation
  const allDates = await loadICTDailyDates(14);
  ...
  // Load multi-day data for streaks
  const datesForStreaks = allDates.slice(currentIdx >= 0 ? currentIdx : 0);
  const multiDayRows = await loadICTDailyMulti(datesForStreaks);
  ...
  if (prevDate) {
    ...
    for (const r of multiDayRows) {
      if (r.scan_date === prevDate) {
        prevScores.set(r.ticker, r.best_score);
      }
    }
    for (const r of results) {
      const prev = prevScores.get(r.ticker);
      if (prev !== undefined) {
        deltas[r.ticker] = r.best_score - prev;
      }
    }
  }

  // Build "dropped" list (was in yesterday, gone today)
  const dropped: Array<{ ticker: string; prev_score: number }> = [];
  if (prevDate) {
    for (const r of multiDayRows) {
      if (r.scan_date === prevDate && !currentTickers.has(r.ticker)) {
        dropped.push({ ticker: r.ticker, prev_score: r.best_score });
      }
    }
