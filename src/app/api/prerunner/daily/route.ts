...
import {
  loadPreRunnerDaily,
  loadPreRunnerDailyDates,
  loadPreRunnerDailyMulti,
...
export async function GET(request: NextRequest) {
  ...
  if (searchParams.get("dates") === "true") {
    const dates = await loadPreRunnerDailyDates(14);
  ...
  const results = await loadPreRunnerDaily(date);

  // Load all available dates to compute streaks + deltas
  const allDates = await loadPreRunnerDailyDates(14);
  ...
  const multiDayRows = await loadPreRunnerDailyMulti(datesForStreaks);
  ...
  if (prevDate) {
    ...
    for (const r of multiDayRows) {
      if (r.scan_date === prevDate) {
        prevScores.set(r.ticker, r.prerunner_score);
      }
    }
    for (const r of results) {
      const prev = prevScores.get(r.ticker);
      if (prev !== undefined) {
        deltas[r.ticker] = r.prerunner_score - prev;
      }
    }
  }
  ...
  if (prevDate) {
    for (const r of multiDayRows) {
      if (r.scan_date === prevDate && !currentTickers.has(r.ticker)) {
        dropped.push({ ticker: r.ticker, prev_score: r.prerunner_score });
      }
    }
