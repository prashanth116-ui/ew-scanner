...
import type { AlertConfig, ConfidenceTier, ScannerMode } from "@/lib/ew-types";
...
async function fetchQuote(ticker: string): Promise<EnrichedQuoteInput | null> {
  try {
...
export async function POST(request: NextRequest) {
  ...
  if (tickers.length === 0) {
    return NextResponse.json({ error: `Unknown universe: ${universe}` }, { status: 400 });
  }
