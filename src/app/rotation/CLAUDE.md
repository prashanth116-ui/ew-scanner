...
**Qualifying-count trajectory (`QualifyingTrend`).** `EntryScreenResult.liveQualifying` re-runs the stock screen on the latest bar and the card and table render it as `screen 5 ↑ 7`. The count is the load-bearing number in the system — 1 name measured 57% positive, 2 measured 30%, 3+ measured 87% — so whether it is building or decaying since the start is worth seeing. **Display only, never a verdict input**, same status as `live`: the verdict stays on the start bar where the study validated it. Stage 6 put the trajectory read at n=50 with a 44-71% Wilson interval, which includes chance, so it informs a reader and gates nothing. Costs no fetch — `fetchStockPerformance` already holds the 6mo chart and calls `computeEntryScreen` a second time on the last index.
