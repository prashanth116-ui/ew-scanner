...
// ── Single-ticker earnings export (used by /earnings detail page) ──

interface SingleEarningsData {
  ticker: string;
  name: string;
...
export async function exportSingleEarningsToExcel(
  ...
  const overview = [
    {
      ...
      "Confluence %": scanResult
        ? Math.round(scanResult.scores.confluenceScore * 100)
        : "-",
  ...
  if (data.history.length > 0) {
    const histRows = data.history.map((h) => ({
      Quarter: h.quarter,
      Date: h.date,
      "EPS Estimate": h.epsEstimate,
      "EPS Actual": h.epsActual,
      "EPS Surprise": h.epsSurprise,
      "Surprise %": h.surprisePercent != null
        ? `${h.surprisePercent.toFixed(1)}%`
        : "-",
      Beat: h.epsActual != null && h.epsEstimate != null
        ? h.epsActual >= h.epsEstimate ? "Yes" : "No"
        : "-",
    }));
  ...
  XLSX.writeFile(
    wb,
    `earnings-${data.ticker}-${new Date().toISOString().slice(0, 10)}.xlsx`
  );
}
