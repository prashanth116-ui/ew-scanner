...
export async function exportPreRunToExcel(results: PreRunResult[]): Promise<void> {
  ...
  const rows = results.map((r) => ({
    ...
    "Dist Days 20d": r.data.distributionDays20d ?? "-",
    "Insider 45d": r.data.insiderBuys45d ?? "-",
    "Data Quality": r.data.dataQuality !== null ? `${r.data.dataQuality}%` : "-",
...
export async function exportVCPToExcel(results: VCPResult[]): Promise<void> {
  ...
  const rows = results.map((r) => {
    ...
    return {
      ...
      "Dist Days 20d": d.distributionDays20d ?? "-",
      "Data Quality": d.dataQuality !== null ? `${d.dataQuality}%` : "-",
...
export async function exportInflectionToExcel(results: InflectionResult[]): Promise<void> {
  ...
  const rows = results.map((r) => {
    ...
    return {
      ...
      "Supply Exhaust": s.supplyExhaustion,
      "Demand Emerge": s.demandEmergence,
      "Compression": s.compression,
      "Runner Potential": s.runnerPotential,
      "RS Trajectory": s.rsTrajectory,
      Stage: r.stage,
